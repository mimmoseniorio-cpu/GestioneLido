...
// L'accesso non conta come interazione dello scenario: si fa una volta
// la mattina, non a ogni cliente che chiede un ombrellone.
await accedi(p, URL)
await p.goto(`${URL}/map`, { waitUntil: 'networkidle' })

// ── 0 interazioni: la risposta è già visibile (scenario F) ────────────────
// Un solo numero, il più grande sullo schermo: quanto si può vendere adesso.
const vendibili = await p.locator('.tile.forte b').innerText()
const occupati = await p.locator('.tile', { hasText: 'occupati' }).locator('b').innerText()
verifica('la mappa dice cosa si può vendere ora, senza toccare nulla', /\d+/.test(vendibili))
verifica('e quanti posti sono occupati', /\d+/.test(occupati))
verifica('96 ombrelloni disegnati', (await p.locator('g.umb').count()) === 96)

const prima = parseInt(vendibili, 10)
...
verifica('un posto in meno da vendere', dopo === prima - 1)
